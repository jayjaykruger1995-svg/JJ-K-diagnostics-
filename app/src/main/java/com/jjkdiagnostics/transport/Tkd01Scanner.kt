...
class Tkd01Scanner(private val context: Context) {
    ...
    fun stopScan() { try { scanCallback?.let { leScanner?.stopScan(it) } } catch (_: Exception) {}; scanCallback = null }
    ...
    fun connect(device: BluetoothDevice, onStatus: (String) -> Unit) {
        ...
        gatt?.close()
        onStatus("Connecting to TKD01…\nType=${device.type} Bond=${device.bondState}")
        gatt = device.connectGatt(context, false, object : BluetoothGattCallback() {
            override fun onConnectionStateChange(g: BluetoothGatt, status: Int, newState: Int) {
                ...
                else if (newState == BluetoothGatt.STATE_DISCONNECTED) onStatus("TKD01 disconnected (GATT status=$status)")
            }
            override fun onServicesDiscovered(g: BluetoothGatt, status: Int) {
                ...
                for (service in g.services) {
                    ...
                    if ((p and BluetoothGattCharacteristic.PROPERTY_NOTIFY) != 0 || (p and BluetoothGattCharacteristic.PROPERTY_INDICATE) != 0) notifyCharacteristic = characteristic
                        onStatus("Characteristic: ${characteristic.uuid} props=${characteristic.properties}")
                        count++
                    }
                }
